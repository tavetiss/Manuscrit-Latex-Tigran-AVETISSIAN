...
\definecolor{blueUSMB}{rgb}{0.0, 0.2, 0.6}   % Bleu logo USMB
\definecolor{a}{rgb}{0.7, 0.1, 0.0}
\definecolor{b}{rgb}{1, 0.5, 0}
\definecolor{c}{rgb}{0.2, 0.5, 0.2}
...
\setcounter{tocdepth}{2}             % Profondeur d'affichange dans la table des matières générale
\setcounter{minitocdepth}{3}         % Profondeur d'affichange dans la table des matières en début de chapitre
\setcounter{secnumdepth}{3}          % Profondeur de numérotation dans la table des matières générale et mini table en début de chapitre
...
\usepackage{float}
\usepackage{graphicx}
\usepackage{placeins}   % placement des figures
%\usepackage{subfigure}
\usepackage{caption}
\usepackage{subcaption} % avoir la même légende pour x figures côte à côte
...
\usepackage{bm}
\usepackage{upgreek} % italiques : $\varphi$ - droites : $\upvarphi$
...
\usepackage{hyperref} %%%  Packages pour les liens hypertexte a mettre après tous les autres packages
 \hypersetup{
 colorlinks=true,
 linkcolor = myblue3,       % Couleur éléments de la table des matières
 citecolor = BleuPetrole,    % Couleur citations
 filecolor = VertVert        % Couleur documents locaux
 }
 
%  hyperindex=true,

% Référencement multiple
